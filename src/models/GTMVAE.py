...
def _kl_normal(mu: torch.Tensor, logvar: torch.Tensor, reduction: str = "mean") -> torch.Tensor:
    """
    KL(q(z|x)=N(mu,diag(sigma^2)) || p(z)=N(0,I)).
    Si reduction='mean', devuelve media por elemento (nodos).
    """
...
class EdgeAwareGCNVEncoder(nn.Module):
    """
    Encoder variacional con pre-bottleneck MLP (in->pre_dim),
    2 capas TransformerConv y normalización. Devuelve (mu, logvar).
    """
    def __init__(self, in_ch, edge_dim, hid=128, out=64, heads=2, dropout=0.2, pre_dim=256, pre_drop=0.1):
        super().__init__()
        # 1) Pre-bottleneck: comprime feats en pre_dim
        self.pre = nn.Sequential(
            nn.Linear(in_ch, pre_dim),
            nn.ReLU(),
            nn.Dropout(pre_drop)
        )
...
def train_edge_node_multitask_v(
    ...
    for ep in range(1, epochs + 1):
        ...
        loss = edge_reg_loss + lambda_node * node_reg_loss + (lambda_rank * rank_loss if add_ranking else 0.0) + kl_coeff * kl_loss
        loss.backward()
        opt.step()
    ...
    model.eval()
    with torch.no_grad():
        mu_full, _ = model.encoder(data.x.to(device), data.edge_index.to(device), data.edge_attr.to(device))
        Z = mu_full.cpu()
    ...
    if print_every:
        print("[TEST]")
        print(f" [EDGE] RMSE={edge_test_rmse:.6f} | MAE={edge_test_mae:.6f} | Sp={edge_test_spr:.6f} | R2={edge_test_r2:.6f}")
        print(f" [NODE] RMSE={node_test_rmse:.6f} | MAE={node_test_mae:.6f} | Sp={node_test_spr:.6f} | R2={node_test_sr2:.6f}")
