...
@torch.no_grad()
def evaluate_gtmae(model, data, node_feat_names, edge_attr_names, target_cols, node_target_cols,
                   seed=33, val_ratio=0.2, test_ratio=0.2, device="cpu", use_pair_feats=True, pair_mode="cosine_l2_absdiff"):
    """Reconstruye los splits con el seed dado y evalúa métricas en VAL y TEST."""
    model.eval()
...
def default_gtmae_param_grid():
    """Define aquí tu malla de hiperparámetros (ajústala a tu compute)."""
    grid = {
        "hid":              [96, 128],
        "out":              [64, 128],
        "heads":            [2, 4],
...
def composite_score(row, w_edge=0.5, w_node=0.5):
    """
    Score para ordenar: menor es mejor.
    Suma ponderada de RMSE en VAL (edge + node).
    Puedes cambiar a algo como:
      - (1 - R2) ponderado
      - o combinar Spearman (1 - rho) si valoras más orden que escala.
    """
...
def run_gtmae_gridsearch(
    ...
    device,
    param_grid=None,
    limit=None,                 # si quieres cortar la malla (debug)
    sort_weights=(0.5, 0.5),    # ponderación edge/node en el score compuesto
):
    torch.cuda.empty_cache() if torch.cuda.is_available() else None

    # Targets: TODAS las columnas
    target_cols = list(edge_attr_names)           # incluye "edge_type" al final en tu prepro
    ...
    for i, hp in enumerate(combos, 1):
        ...
        try:
            ...
            print(f"[{desc}] score={row['score']:.4f}  edge_RMSE={row['val_edge_rmse']:.4f}  node_RMSE={row['val_node_rmse']:.4f}")
