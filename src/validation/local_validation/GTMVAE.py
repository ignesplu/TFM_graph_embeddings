...
from ...models.GTMVAE import train_edge_node_multitask_v
...
def evaluate_gtmvae(
    ...
    """
    Evaluate GTMVAE model performance on validation and test sets using deterministic mode.
    ...
    Uses deterministic encoding (mean) rather than stochastic sampling.

    Args:
        model: Trained GTMVAE model
...
def default_gtmvae_param_grid():
    """
    Define default hyperparameter grid for GTMVAE model optimization.

    Returns a comprehensive parameter grid specifically tuned for the
    Graph Transformer Multitask Variational Autoencoder architecture,
    including additional variational parameters (beta_kl, kl_warmup).
...
def run_gtmvae_gridsearch(
    ...
    """
    Execute comprehensive grid search for GTMVAE hyperparameter optimization.

    Performs systematic hyperparameter search across specified parameter grid
    for the Graph Transformer Multitask Variational Autoencoder. Trains models
    with different configurations and evaluates performance using composite scoring.
    Tracks best performing model and deterministic embeddings.
    ...
    Returns:
        Tuple of (results DataFrame, best model, deterministic embeddings)
    """
    ...
    for i, hp in enumerate(combos, 1):
        ...
        try:
            ...
            print(
                f"[*] score={row['score']:.4f}  edge_RMSE={row['val_edge_rmse']:.4f}  node_RMSE={row['val_node_rmse']:.4f}"
            )
